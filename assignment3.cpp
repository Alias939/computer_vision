		...
		int argc, char** argv)
{
    ...
    //{
    //    VideoCapture cap("data/cam" + string(to_string(i))+string("/background.avi"));
    //    if (!cap.isOpened())
    //    {
    //        return false;
    //    }
    //    Mat frame;
    //    cap >> frame;
    //    imwrite("data/cam" + string(to_string(i)) + string("/background.png"), frame);
    //}
 /*   Mat frame;
    Mat corners;
    frame = imread("4persons/extrinsics/ex4.jpg");
    bool found = findChessboardCorners(frame, Size(8, 6), corners,3);
    
    drawChessboardCorners(frame, Size(8, 6), corners, found);
    imshow("frame", frame);
    waitKey(0);*/
